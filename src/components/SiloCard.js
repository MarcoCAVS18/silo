...
const calculateColor = (value, isDarkMode) => {
  if (value === 0.0) return isDarkMode ? "#FFFFFF" : "#000000";
    ...
    <div className={`bg-white dark:bg-gray-800 shadow-xl rounded-lg p-6 transition-colors duration-300 border-2 border-gray-300 dark:border-gray-700 hover:shadow-2xl transform hover:scale-105 h-64 flex flex-col justify-between relative pb-16 ${isAnimating ? 'slideInUp' : ''}`}>
      <div className="flex flex-col justify-center items-center flex-1">
        <div className="flex justify-between items-center w-full pt-10">
          <div className="flex flex-col space-y-4">
            ...
            <div className="text-3xl font-bold select-none" style={{ color: meterColor }}>
      ...
      {isEditing && (
        <div className="absolute bottom-4 left-0 right-0 flex flex-row space-x-4 px-6">
          <input type="number" inputMode="decimal" value={meters} onChange={(e) => setMeters(Math.min(31, parseFloat(e.target.value)))} className="w-1/2 p-3 border rounded dark:bg-gray-700 dark:text-white" placeholder="0.00" />
          <input type="text" value={kind} onChange={(e) => setKind(e.target.value)} className="w-1/2 p-3 border rounded dark:bg-gray-700 dark:text-white" placeholder="Type kind" />
          <button onClick={handleSave} className={`px-4 py-2 rounded ${meters !== "" && kind.trim() !== "" && !isNaN(meters) ? "bg-green-500 text-white hover:bg-green-600" : "bg-gray-400 text-gray-700 cursor-not-allowed"}`} disabled={meters === "" || kind.trim() === "" || isNaN(meters)}>
            Save
          </button>
        </div>
      )}
      ...
      <div className="absolute top-4 w-full flex flex-wrap gap-2 capitalize opacity-70">
        {kind?.trim() && <Tag label={kind} color="#007BFF" />}
        {tagDetails && <Tag label={tagDetails.label} color={tagDetails.color} />}
      </div>
    </div>
  );
};
